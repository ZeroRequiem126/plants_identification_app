<?php
    session_start();
    require('../assets/templates/library.php');

    $PDO = dbconnect();

    $tree_id = $_SESSION['tree_id'];

    if (!empty($_POST['date']) && !empty($_POST['memo']) && $tree_id && !empty($_SESSION['login'])) {
        try {
            $date = $_POST['date'];
            $memo = $_POST['memo'];

            $sql = "INSERT INTO memos (date, memo, user_id, tree_id) VALUES (:date, :memo, '".$_SESSION['login']."', '".$tree_id."')";
            $stmt = $PDO->prepare($sql);
            $params = array(':date' => $date, ':memo' => $memo);
            $stmt->execute($params);

            header('Location: ../memo/memos.php');
            exit();
        } catch (PDOExeption $e) {
            exit('データベースエラー');
        }
    } else {
        $message = "全ての項目を入力してください";
    }
?>
<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <link rel="stylesheet" href="../assets/style/style.css">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
    <title>メモ登録</title>
</head>
<body>

<?php include("../assets/templates/header.html"); ?>

<?php
    echo '<div class="memo-post-page">';
    echo '<h2>メモ登録</h2>';
    echo '<div class="container">';
    echo '<form action="" method="post">';
    echo '<div class="mb-3">';
    echo '<h3>日付</h3>';
    echo '<input type="date" name="date" class="form-control" required>';
    echo '</div>';
    echo '<div class="mb-3">';
    echo '<h3>メモ内容</h3>';
    echo '<textarea name="memo" rows="5" class="form-control" required></textarea>';
    echo '</div>';
    echo '<div class="buttons">';
    echo '<button type="submit" class="btn btn-lg btn-danger" id="signup-button">登録</button>';
    echo '<a href="../tree/favorites.php" class="btn btn-lg btn-dark">お気に入り一覧</a>';
    echo '</div>';
    echo '</form>';
    echo '</div>';
    echo '</div>';
?>

<?php include("../assets/templates/footer.html"); ?>

<script src="https://code.jquery.com/jquery-3.6.0.min.js" integrity="sha256-/xUj+3OJU5yExlq6GSYGSHk7tPXikynS7ogEvDej/m4="crossorigin="anonymous"></script>
<script src="../assets/js/main.js"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
</body>
</html>